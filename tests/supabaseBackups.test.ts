...
describe("Supabase backups", () => {
  ...
  it("allows expired snapshots with internal references to cascade atomically", () => {
    const migration = readFileSync(
      new URL(
        "../supabase/migrations/20260817130000_defer_snapshot_internal_foreign_keys.sql",
        import.meta.url
      ),
      "utf8"
    );
    for (const constraint of [
      "routine_items_snapshot_id_exercise_id_fkey",
      "daily_session_routines_snapshot_id_routine_id_fkey",
      "session_exercises_snapshot_id_source_exercise_id_fkey"
    ]) {
      expect(migration).toContain(`drop constraint ${constraint}`);
      expect(migration).toContain(`add constraint ${constraint}`);
    }
    expect(migration.match(/on delete no action/g)).toHaveLength(3);
  ...
  it("stores a snapshot in one database request and relies on server retention", async () => {
    const rpc = vi.fn((name: string) =>
      Promise.resolve({
        data: name === "get_my_app_role" ? "user" : "backup-new",
        error: null
      })
    );
    const client = {
    ...
    await expect(backups.upload(appData)).resolves.toMatchObject({ id: "backup-new" });
  ...
  it("uploads only changed entities after the initial snapshot", async () => {
    const rpc = vi.fn((name: string) =>
      Promise.resolve({
        data: name === "get_my_app_role" ? "user" : "snapshot-2",
        error: null
      })
    );
    const client = {
